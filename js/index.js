...
// Editing the information of the user through the session variable
...
let sessionPhoneNo = sessionStorage.getItem("phone");

let id = sessionStorage.getItem("id");
userEmail.setAttribute("href", "mailto:" + userEmail.innerHTML);
templateUserName.innerHTML = sessionStorage.getItem("username");
phoneNo.setAttribute("href", "//api.whatsapp.com/send?phone=" + sessionPhoneNo);


function userIsLoggedIn() {
    let sessionVariable = sessionStorage.getItem("email");
...
let projectDiv = document.getElementById("project-main-div");
...
function saveProject(projectData) {
    let temp = JSON.parse(localStorage.getItem("projects")) || [];

    let existingIndex = temp.findIndex(existingProject => existingProject.projectNameData === projectData.projectNameData);
    if (existingIndex !== -1) {
        temp[existingIndex] = projectData;
    } else {
...
function projectDataToAppend(projectData) {
    ...
    if (projectData.projectID === id) {
        ...
        let projectMainDiv = document.getElementById("project-main-div");
...
function appendProject() {
    let getProjectData = JSON.parse(localStorage.getItem("projects")) || [];

    for (let eachData of getProjectData) {
        projectDataToAppend(eachData);
    }
    viewProject();
...
function editProject() {
    let projectSection = document.getElementById("project-main-div");
    projectSection.addEventListener("click", (e) => {
        ...
        if (target.classList.contains("edit-project")) {
            ...
            // let projectsArrayTags = JSON.parse(localStorage.getItem("projects"));
            // let tagsToUpdate = projectsArrayTags.findIndex(project => project.projectNameData === projectName);

            // let langTags;
            // if (tagsToUpdate) {
            //     let projectTags = projectsArrayTags[tagsToUpdate];
            //     langTags = projectTags.tags;
            // }
            ...
            // document.getElementById("tags").value = langTags.join(", ");
            ...
            projectForm.addEventListener("submit", (e) => {
                ...
                let projectToUpdate = projectsArray.find(project => project.projectNameData === projectName);
                ...
                if (projectToUpdate) {
                    ...
                    projectToUpdate.projectNameData = document.getElementById("projectName").value;
                    projectToUpdate.projectDescription = document.getElementById("projectDescription").value;
                    projectToUpdate.tags = document.getElementById("tags").value.split(',').map(tag => tag.trim());
                    projectToUpdate.projectLiveLink = document.getElementById("linkToLiveWebsite").value;
                    projectToUpdate.projectRepo = document.getElementById("linkToRepo").value;
                    projectToUpdate.projectImg = dataURL;
                    ...
                    tags.innerHTML = document.getElementById("tags").value.split(',').map(tag => tag.trim());
...
function deleteProject() {
    ...
    projectSection.addEventListener("click", (e) => {
        ...
        if (target.classList.contains("delete-project")) {
            ...
            let existingIndex = projectsArray.findIndex(existingProject => existingProject.projectNameData === heading.innerHTML);

            if (existingIndex !== -1) {
                projectsArray.splice(existingIndex, 1);
                localStorage.setItem("projects", JSON.stringify(projectsArray));
    ...
    document.addEventListener("click", (e) => {
        ...
        if (element.classList.contains("view-project")) {
            ...
            for (let eachproject of fetchusersProject) {
                if (eachproject.projectNameData === projectName) {
                    ...
                    for (let eachTag of eachproject.tags) {
                    ...
                    pDescription.textContent = eachproject.projectDescription;
                }
            }
        }
    });
...
function saveData(newData) {
    let id = newData.userID;
    let p = newData.position;
    let a = newData.aboutMe;

    let temp = JSON.parse(localStorage.getItem("editable-content")) || [];
    ...
    localStorage.setItem("editable-content", JSON.stringify(temp));
}

let editableData = document.querySelectorAll(".editable-element");
function appendNewEditedData(data) {
    for (let eachData of data) {
        if (eachData.userID === id) {
            editableData[0].innerHTML = eachData.position;
            editableData[1].innerHTML = eachData.aboutMe;
        }
    }
}

function storeContentEditabaleData() {
    document.addEventListener("click", (e) => {
        let selectedElement = e.target;
...
let storedData = JSON.parse(localStorage.getItem("editable-content")) || [];
appendNewEditedData(storedData);
storeContentEditabaleData();
